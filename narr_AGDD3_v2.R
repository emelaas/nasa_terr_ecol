...
system.time({
  ...
  for (i in 1:75){
    for (j in 1:24){
      ...
      cell_r3 <- matrix(NA,nrow(obs.SPR),1)
      cell_ST3 <- matrix(NA,nrow(obs.SPR),1)
      cell_mk <- matrix(NA,nrow(obs.SPR),1)
      cell_z <- matrix(NA,nrow(obs.SPR),1)
      for (k in 1:nrow(obs.SPR)){
        if (length(which(is.na(obs.SPR[k,1:32])==0))>5){
          lm.pred <- lm(obs.SPR[k,1:32]~pred.SPR[1:32,k])
          cell_ST3[k] <- round(coef(lm.pred),3)[2]
          cell_r3[k] <- cor(obs.SPR[k,1:32],pred.SPR[1:32,k],use='pairwise.complete.obs')
          
          mk <- MannKendall(pred.SPR[1:32,k])
          cell_mk[k] <- as.numeric(mk$sl)
          
          y <- as.numeric(pred.SPR[1:32,k])
      ...
      site_ST3[i,j] <- mean(cell_ST3,na.rm=TRUE)
      site_cor3[i,j] <- mean(cell_r3,na.rm=TRUE)
  ...
  save(site_ST3,site_cor3,ST3,COR3,scenes,site_z,Z,w,file = "preseason_temp_cor_gdd")
